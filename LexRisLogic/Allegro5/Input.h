...
namespace LL_AL5
{
    struct LL_Key
    {
        ...
        int keycode;
        bool active=0;
        LL_Key(std::string n,int k){name=n;keycode=k;}
    };
    ...
    {
        private:
            std::vector<LL_Key> key;
        public:
            LL_Key& operator [] (unsigned int index){return key[index];}
            int find_key(std::string Name){for(unsigned int i=0;i<key.size();++i){if(key[i].name==Name)return i;}return -1;}
            int find_key(int keycode){for(unsigned int i=0;i<key.size();++i){if(key[i].keycode==keycode)return i;}return -1;}
            bool add_key(std::string Name,int keycode){if((find_key(Name)!=-1) or (find_key(keycode)!=-1))return 0;key.push_back(LL_Key(Name,keycode));return 1;}
            bool mod_key(unsigned int index,std::string new_Name){if((index<key.size()) and (find_key(new_Name)==-1)){key[index].name=new_Name;return 1;}return 0;}
            ...
            bool remove_key(std::string Name){int lol=find_key(Name);if(lol==-1)return 0;key.erase((key.begin())+lol);return 1;}
    ...
    class LL_Input
    {
        ...
        public:
            ...
            bool& operator [] (std::string Name){_ff=0;int lol=_KeySearchName(Name);if(lol==-1)return _ff;return ((*key)[lol]).active;}
            void operator()();
            void get_exit();
