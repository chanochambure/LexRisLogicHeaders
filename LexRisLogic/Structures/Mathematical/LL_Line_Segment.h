#ifndef LL_LINE_SEGMENT_H_INCLUDED
#define LL_LINE_SEGMENT_H_INCLUDED
...
class LL_LineSegment
{
    ...
    public:
        ...
        bool num_in_interval_x(float num){return (min(p1[0],p2[0])<=num and num<=max(p1[0],p2[0]));}
        bool num_in_interval_y(float num){return (min(p1[1],p2[1])<=num and num<=max(p1[1],p2[1]));}
};
...
#endif // LL_LINE_SEGMENT_H_INCLUDED
